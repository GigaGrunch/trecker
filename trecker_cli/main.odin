...
command_init :: proc() {
    if os.exists(tl.SESSION_FILE_PATH) {
        fmt.printfln("Session file already exists at '%v'.", tl.SESSION_FILE_PATH)
        os.exit(1)
    }
    
    store := tl.Store{}
    write_ok := tl.write_store_file(store)
    if !write_ok do os.exit(1)
}

command_add :: proc(args: Add_Args) {
    store, store_ok := tl.read_store_file()
    defer tl.store_destroy(&store)
    ...
    tl.store_add_project(&store, args.project_id, args.project_name)
...
command_start :: proc(args: Start_Args) {
    ...
    for p in store.projects {
        initial_today_duration += tl.get_today_duration(store, p)
    }
    
    entry := tl.store_add_entry(&store, args.project_id, time.now(), time.now())
...
summary_make :: proc(args: Summary_Args, store: tl.Store) -> Summary {
